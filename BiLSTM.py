...
class BiLSTMModel(nn.Module):
    def __init__(self, size = 256, embedding_dim = 100, num_layers = 2, dropout = 0.2, number_of_output_classes = 2, L = 30, prediction_method = 'sum', learning_rate=0.0001):
        ...
        self.fc = nn.Sequential(nn.Linear(2 * self.lstm_size, number_of_output_classes))
    ...
    def init_state(self):
        return (torch.zeros(2 * self.num_layers, self.L, self.lstm_size),
                torch.zeros(2 * self.num_layers, self.L, self.lstm_size))
